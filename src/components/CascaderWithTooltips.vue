...
export default {
  ...
  data() {
    return {
      options: [
        {
          value: "1",
          label: "非常非常非常长的选项文本示例 1——超过显示宽度时应显示 tooltip",
        },
        {
          value: "2",
          label: "第二项（也很长）",
          children: [
            {
              value: "2-1",
              label: "子项文本也很长，需要 tooltip 来完整展示示例",
            },
  ...
  mounted() {
    ...
    this._observer = new MutationObserver((muts) => {
      for (const m of muts) {
        for (const node of m.addedNodes) {
          ...
          // 菜单的根节点或包含菜单的容器
          if (
            node.classList.contains("ivu-cascader-menu") ||
            (node.querySelector && node.querySelector(".ivu-cascader-menu"))
          ) {
            console.log(node);
            // 延迟一个渲染帧，确保菜单项已渲染
    ...
    this._observer.observe(document.getElementById("container"), {
      childList: true,
      subtree: true,
    });
  },
  ...
  methods: {
    _applyTitles(rootNode) {
      const menuRoot = rootNode.classList.contains("ivu-cascader-menu")
        ? rootNode
        : rootNode.querySelector(".ivu-cascader-menu");
      if (!menuRoot) return;
